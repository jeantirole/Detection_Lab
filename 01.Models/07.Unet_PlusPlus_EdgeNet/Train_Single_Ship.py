...
#--- logger
# Set up logging
log_filename = datetime.datetime.now().strftime(f'/mnt/hdd/eric/.tmp_ipy/15.Lab_Detection/01.Models/07.Unet_PlusPlus_EdgeNet/01.log/ver_{EXEC_VER}_%Y-%m-%d_%H-%M-%S.log')
...
for epoch in range(epochs):
    ...
    for index, data in enumerate(dataloader):
        ...
        imgs, masks, stacked_mask = data
        imgs, masks, stacked_mask = imgs.to(DEVICE), masks.to(DEVICE), stacked_mask.to(DEVICE)
    
        # opt
        optimizer.zero_grad()
        
        # runs
        outputs,loss_percept = model(imgs, stacked_mask)
        
        # loss
        loss_seg,dice_score = criterion(outputs, masks)
