...
parser.add_argument('--cfg', type=str, default='/mnt/hdd/eric/.tmp_ipy/15.Lab_Detection/07.Challenge/01.MapYourCity_HuggingFace/configs/finetune_10.yaml')
...
for epoch in range(cfg.EPOCHS):
    ...
    for iteration, (str_imgs,top_imgs, _, labels) in enumerate(trainloader):
        ...
        if cfg.LOSS_FN == "MSE" or cfg.LOSS_FN == "MAE" :
            ...
            labels = torch.tensor([RS_utils.label_to_dist_torch(la) for la in labels], dtype=torch.float32)
            label_deviation = cfg.LABEL_DEVIATION
    ...
    if cfg.LOSS_FN == "CE" or "KL":
        ...
        #-- evaluate validation
        valid_precision, valid_recall, valid_f1, valid_accuracy = map_train.test_v4_1(testloader, model, val_loss_fn,cfg,metric_obj)
        ...
        #-- evaluate validation
        regression_loss = map_train.test_v4_1(testloader, model, val_loss_fn,cfg,metric_obj)
    ...
    if cfg.LOSS_FN=="MSE" or cfg.LOSS_FN=="MAE":
        ...
        torch.save(model.state_dict(), os.path.join(cfg.SAVE_DIR, f"{cfg.RUN_VERSION}_{cfg.MODEL}_{cfg.LOSS_FN}_f1_{valid_f1}_epoch_{epoch}.pth"))
