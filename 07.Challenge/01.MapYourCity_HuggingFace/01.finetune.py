...
three types of regression loss added 

1.mae = nn.L1Loss()
2.mse = nn.MSELoss()
3.mpe = torch.mean(torch.abs((target - output) / target))
...
parser.add_argument('--cfg', type=str, default='./configs/finetune_7.yaml')
...
if cfg.TIMM:
    train_set = map_dataset.Map_Dataset_v5(names_train,train_path,max_size=data_config['input_size'][1],cfg=cfg,split='train') 
...
for epoch in range(cfg.EPOCHS):
    ...
    if cfg.LOSS_FN == "CE":
        ...
        #-- evaluate validation
        regression_loss = map_train.test_v3(testloader, model, val_loss_fn,cfg,metric_obj)
