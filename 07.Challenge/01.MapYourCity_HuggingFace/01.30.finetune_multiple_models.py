...
parser.add_argument('--cfg', type=str, \
                    default='/mnt/hdd/eric/.tmp_ipy/15.Lab_Detection/07.Challenge/01.MapYourCity_HuggingFace/configs/finetune_24.yaml')
...
for fold, (train_ids, valid_ids) in enumerate(SKF.split(names_data,names_label)):
    ...
    multimodal = RS_models.EnsembleModel(model_1, model_2, model_3)
    model = multimodal
    model.train()
    ...
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=2, T_mult=1, eta_min=cfg.LR)
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        # Train 
        for iteration, (str_imgs, top_imgs, sentinel_imgs, dist_labels,labels) in enumerate(trainloader):
        ...
        if cfg.LOSS_FN == "CE" or "KL":
            ...
            precision_,recall_, f1_, acc_  = map_train.test_v9(trainloader, model, val_loss_fn,cfg,metric_obj_train)
            valid_precision, valid_recall, valid_f1, valid_accuracy = map_train.test_v9(testloader, model, val_loss_fn,cfg,metric_obj_valid)
            ...
            #---- scheduler step 
            cri = recall_result / len(cfg.DEVICES)
            scheduler.step(cri) 
