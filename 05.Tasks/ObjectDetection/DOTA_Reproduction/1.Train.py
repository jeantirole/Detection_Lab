...
#-- logger 
log_path = f'/mnt/hdd/eric/.tmp_ipy/15.Lab_Detection/05.Tasks/ObjectDetection/Mask_n_Face/01.log/ver_{EXEC_VER}_%Y-%m-%d_%H-%M-%S.log'
...
train_dataset = RS_dataset.MaskDataset(data_transform, '/mnt/hdd/eric/.tmp_ipy/00.Data/FaceMaskDetection/images/')
test_dataset = RS_dataset.MaskDataset(data_transform, '/mnt/hdd/eric/.tmp_ipy/00.Data/FaceMaskDetection/test_images/')

train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn, shuffle=DATA_SHUFFLE)
test_data_loader = torch.utils.data.DataLoader(test_dataset, batch_size=2, collate_fn=collate_fn,shuffle=False)
